...
class SignatureHelpProvider {
    ...
    provideHover(lc, document, position, token) {
        return new Promise((resolve, reject) => {
            lc.sendRequest(vscode_languageclient_1.HoverRequest.type, lc.code2ProtocolConverter.asTextDocumentPositionParams(document, position.translate(0, -1)), token).then(data => resolve(lc.protocol2CodeConverter.asHover(data)), error => reject(error));
        });
    }
    hoverToSignatureHelp(hover, position, document) {
        /*
        ...
        */
        ...
        const label = hover.contents[0].value
            .replace('```rust', '')
            .replace('```', '');
        ...
        const doc = hover.contents.length > 1
            ? hover.contents.slice(-1)[0]
            : undefined;
