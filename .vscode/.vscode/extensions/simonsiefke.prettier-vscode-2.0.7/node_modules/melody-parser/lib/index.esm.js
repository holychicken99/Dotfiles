import { SequenceExpression, PrintExpressionStatement, StringLiteral, PrintTextStatement, TwigComment, HtmlComment, Declaration, Identifier, Element, BinaryConcatExpression, Attribute, NullLiteral, BooleanLiteral, CallExpression, NumericLiteral, ConditionalExpression, ArrayExpression, ObjectExpression, is, ObjectProperty, MemberExpression, SliceExpression, FilterExpression, NamedArgumentExpression, GenericTwigTag, GenericToken } from 'melody-types';
...
var GenericTagParser = {
    ...
    parse: function parse(parser) {
        ...
        var twigTag = new GenericTwigTag(tokens.la(-1).text);
            ...
            if (currentToken.type === TAG_END) {
                ...
                try {
                    ...
                    if (e.errorType === 'UNEXPECTED_TOKEN') {
                        twigTag.parts.push(new GenericToken(e.tokenType, e.tokenText));
                        tokens.next();
                    } else {
                        throw e;
                    }
...
var Parser = function () {
    ...
    Parser.prototype.parse = function parse() {
        ...
        var tokens = this.tokens,
            p = setStartFromToken(new SequenceExpression(), tokens.la(0));
        while (!tokens.test(EOF)) {
            var token = tokens.next();
            if (!p) {
                p = setStartFromToken(new SequenceExpression(), token);
            }
            ...
            switch (token.type) {
                case EXPRESSION_START:
                    {
                        ...
                        var statement = new PrintExpressionStatement(expression);
                ...
                case TEXT:
                    {
                        var textStringLiteral = createNode(StringLiteral, token, token.text);
                        var textTextStatement = createNode(PrintTextStatement, token, textStringLiteral);
                        p.add(textTextStatement);
                        break;
                    }
                case ENTITY:
                    {
                        var entityStringLiteral = createNode(StringLiteral, token, !this.options.decodeEntities || this.options.preserveSourceLiterally ? token.text : decode(token.text));
                        var entityTextStatement = createNode(PrintTextStatement, token, entityStringLiteral);
                ...
                case COMMENT:
                    if (!this.options.ignoreComments) {
                        var stringLiteral = createNode(StringLiteral, token, token.text);
                        var twigComment = createNode(TwigComment, token, stringLiteral);
                        p.add(twigComment);
                    }
                    break;
                case HTML_COMMENT:
                    if (!this.options.ignoreHtmlComments) {
                        var _stringLiteral = createNode(StringLiteral, token, token.text);
                        var htmlComment = createNode(HtmlComment, token, _stringLiteral);
                        p.add(htmlComment);
                    }
                    break;
            }
    ...
    Parser.prototype.matchDeclaration = function matchDeclaration() {
        ...
        var declaration = new Declaration(declarationType.text);
        while (currentToken = tokens.next()) {
            if (currentToken.type === SYMBOL) {
                var symbol = createNode(Identifier, currentToken, currentToken.text);
                declaration.parts.push(symbol);
            } else if (currentToken.type === STRING_START) {
                var stringToken = tokens.expect(STRING);
                declaration.parts.push(createNode(StringLiteral, stringToken, stringToken.text));
                tokens.expect(STRING_END);
            } else if (currentToken.type === EXPRESSION_START) {
                var expression = this.matchExpression();
                declaration.parts.push(copyLoc(new PrintExpressionStatement(expression), expression));
    ...
    Parser.prototype.matchAttributes = function matchAttributes(element, tokens) {
        while (tokens.lat(0) !== SLASH && tokens.lat(0) !== ELEMENT_END) {
            ...
            if (key) {
                var keyNode = new Identifier(key.text);
                ...
                if (tokens.nextIf(ASSIGNMENT)) {
                    ...
                    while (!tokens.test(STRING_END)) {
                        if (canBeString && (token = tokens.nextIf(STRING))) {
                            nodes[nodes.length] = createNode(StringLiteral, token, token.text);
                            canBeString = false;
                    ...
                    tokens.expect(STRING_END);
                    if (!nodes.length) {
                        var node = createNode(StringLiteral, start, '');
                        nodes.push(node);
                    }
                    ...
                    for (var i = 1, len = nodes.length; i < len; i++) {
                        ...
                        expr = new BinaryConcatExpression(expr, nodes[i]);
                    ...
                    var attr = new Attribute(keyNode, expr);
                    copyStart(attr, keyNode);
                    copyEnd(attr, expr);
                    element.attributes.push(attr);
                } else {
                    element.attributes.push(copyLoc(new Attribute(keyNode), keyNode));
                }
    ...
    Parser.prototype.matchPrimaryExpression = function matchPrimaryExpression() {
        ...
        switch (token.type) {
            case NULL:
                node = createNode(NullLiteral, tokens.next());
                break;
            case FALSE:
                node = createNode(BooleanLiteral, tokens.next(), false);
                break;
            case TRUE:
                node = createNode(BooleanLiteral, tokens.next(), true);
                break;
            case SYMBOL:
                tokens.next();
                if (tokens.test(LPAREN)) {
                    // SYMBOL '(' arguments* ')'
                    node = new CallExpression(createNode(Identifier, token, token.text), this.matchArguments());
                    copyStart(node, node.callee);
                    setEndFromToken(node, tokens.la(-1)); // ')'
                } else {
                    node = createNode(Identifier, token, token.text);
                }
                break;
            case NUMBER:
                node = createNode(NumericLiteral, token, Number(tokens.next()));
                break;
            case STRING_START:
    ...
    Parser.prototype.matchStringExpression = function matchStringExpression() {
        ...
        if (!nodes.length) {
            return setEndFromToken(createNode(StringLiteral, stringStart, ''), stringEnd);
        }
    ...
    Parser.prototype.matchConditionalExpression = function matchConditionalExpression(test) {
        ...
        while (tokens.nextIf(QUESTION_MARK)) {
            ...
            condition = new ConditionalExpression(condition, consequent, alternate);
    ...
    Parser.prototype.matchMap = function matchMap() {
        ...
        while (!tokens.test(RBRACKET) && !tokens.test(EOF)) {
            ...
            if (tokens.test(STRING_START)) {
                ...
                if (!is(key, 'StringLiteral')) {
                    computed = true;
                }
            } else if (token = tokens.nextIf(SYMBOL)) {
                key = createNode(Identifier, token, token.text);
            } else if (token = tokens.nextIf(NUMBER)) {
                key = createNode(NumericLiteral, token, Number(token.text));
            } else if (tokens.test(LPAREN)) {
            ...
            var prop = new ObjectProperty(key, value, computed);
            copyStart(prop, key);
            copyEnd(prop, value);
    ...
    Parser.prototype.matchSubscriptExpression = function matchSubscriptExpression(node) {
        ...
        if (op.type === DOT) {
            ...
            if (token.type === SYMBOL) {
                property = createNode(Identifier, token, token.text);
            } else if (token.type === NUMBER) {
                property = createNode(NumericLiteral, token, Number(token.text));
                computed = true;
            } else {
                this.error({
                    title: 'Invalid token',
            ...
            var memberExpr = new MemberExpression(node, property, computed);
            copyStart(memberExpr, node);
            copyEnd(memberExpr, property);
            if (tokens.test(LPAREN)) {
                var callExpr = new CallExpression(memberExpr, this.matchArguments());
            ...
            if (arg) {
                return setEndFromToken(copyStart(new MemberExpression(node, arg, true), node), tokens.expect(RBRACE));
            } else {
                // slice
                var result = new SliceExpression(node, start, tokens.test(RBRACE) ? null : this.matchExpression());
    ...
    Parser.prototype.matchFilterExpression = function matchFilterExpression(node) {
        ...
        while (!tokens.test(EOF)) {
            var token = tokens.expect(SYMBOL),
                name = createNode(Identifier, token, token.text),
                args = void 0;
            if (tokens.test(LPAREN)) {
            ...
            var newTarget = new FilterExpression(target, name, args);
    ...
    Parser.prototype.matchArguments = function matchArguments() {
        ...
        while (!tokens.test(RPAREN) && !tokens.test(EOF)) {
            if (tokens.test(SYMBOL) && tokens.lat(1) === ASSIGNMENT) {
                ...
                var arg = new NamedArgumentExpression(createNode(Identifier, name, name.text), value);
                copyEnd(arg, value);
                args.push(arg);
            } else {
...
export { Parser, TokenStream, Lexer, EOF$1 as EOF, CharStream, LEFT, RIGHT, parse, createExtendedLexer, createExtendedParser, setStartFromToken, setEndFromToken, copyStart, copyEnd, copyLoc, getNodeSource, createNode, hasTagStartTokenTrimLeft, hasTagEndTokenTrimRight, TokenTypes as Types };
